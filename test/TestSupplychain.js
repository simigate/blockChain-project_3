...
contract('SupplyChain', function (accounts) {
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async () => {
        ...
        let balanceOfConsumerBeforeTransaction = await web3.eth.getBalance(distributorID);
        let customerSent = web3.toWei(3, "ether");

        await supplyChain.buyItem(upc, { from: distributorID, value: customerSent, gasPrice: 0 })

        let balanceOfFarmerAfterTransaction = await web3.eth.getBalance(originFarmerID);
        let balanceOfConsumerAfterTransaction = await web3.eth.getBalance(distributorID);
        // Retrieve the just now saved item from blockchain by calling function fetchItem()
        const resultBufferOne = await supplyChain.fetchItemBufferOne.call(upc)
        const resultBufferTwo = await supplyChain.fetchItemBufferTwo.call(upc)
        ...
        assert.equal(eventEmitted, true, 'Invalid event emitted')
        let value1 = Number(balanceOfFarmerAfterTransaction) - Number(balanceOfFarmerBeforeTransaction);
        assert.equal(value1, productPrice, 'Invalid farmer transaction')
        let value2 = Number(balanceOfConsumerBeforeTransaction) - Number(balanceOfConsumerAfterTransaction);
        assert.equal(value2, productPrice, 'Invalid consumer transaction')
